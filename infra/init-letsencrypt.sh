...
for domain in ${!domains[*]}; do
  ...
  if [ ! -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    sudo docker compose -f $data_path/docker-compose.yml run --rm --entrypoint "openssl req -x509 -nodes -newkey rsa:1024 \
...
sudo docker compose -f $data_path/docker-compose.yml up -d nginx && sudo docker compose -f $data_path/docker-compose.yml restart nginx
